...
let Blobby = (() => {
  ...
  let tilt = [0, 0];
  let gravity_magnitude = 0.4;
  let gravity_force = [0, 0.4];
  let landscape = false;
  let staticdevice = true;
  ...
  window.addEventListener("devicemotion", e => {
    staticdevice = false;
    tilt[0] += e.acceleration.x;
    tilt[1] += e.acceleration.y;
  });

  window.addEventListener("deviceorientation", e => {
    gravity_force[0] = e.gamma / 90 * gravity_magnitude;
    gravity_force[1] = e.beta / 90 * gravity_magnitude;
  });
  ...
  function moveBlobs() {
    for (let i = 0; i <= particlesNum; i++) {
      ...
      if (gravity) {
        velocity_x[i] += gravity_force[0];
        velocity_y[i] += gravity_force[1];
        velocity_x[i] += tilt[0] * 0.4;
        velocity_y[i] += tilt[1] * 0.4;
      }
  ...
  function borderConstraint() {
    ...
    for (let i = 0; i < particlesNum; i++) {
      ...
      if (dist > limit_radius) {
        ...
        if (gravity && staticdevice) {
          if (position_x[i] > limit_center[0] + blob_size) {
            prev_x[i] = position_x[i] + 2 + 0.2 * velocity_x[i];
          } else if (position_x[i] < limit_center[0] - blob_size) {
            prev_x[i] = position_x[i] - 2 + 0.2 * velocity_x[i];
          }
        }
      }
...
// Add button function to toggle gravity
// document.getElementById("gravity").onclick = function() {toggleGravity();}
